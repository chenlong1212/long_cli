...
public class LongTermMemory implements Memory {
    ...
    private static File resolveStorageDir() {
        String configuredDir = System.getProperty("longcli.memory.dir");
        if (configuredDir == null || configuredDir.isBlank()) {
            configuredDir = System.getenv("LONGCLI_MEMORY_DIR");
        }
        if (configuredDir != null && !configuredDir.isBlank()) {
            return new File(configuredDir);
        }
        return new File(new File(System.getProperty("user.home"), ".longcli"), "memory");
    }
