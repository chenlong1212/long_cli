...
public class MemoryRetriever {
    ...
    public List<MemoryEntry> retrieveLongTerm(String query, int limit, String projectKey) {
        // 第一步：获取所有可见的记忆
        List<MemoryEntry> visible = longTermMemory.getAll().stream()
        ...
        // 第二步：尝试匹配
        List<MemoryEntry> matched = visible.stream()
        ...
        // 如果没有匹配到，但有事实型记忆，返回事实
        if (matched.isEmpty()) {
            return visible.stream()
                    .filter(e -> e.getType() == MemoryEntry.MemoryType.FACT)
                    .limit(limit)
                    .collect(Collectors.toList());
        }
