...
public class PlanExecuteAgent {
    ...
    public PlanExecuteAgent(LlmClient llmClient, MemoryManager memoryManager) {
        this(llmClient, new ToolRegistry(), memoryManager);
    }

    public PlanExecuteAgent(LlmClient llmClient, ToolRegistry toolRegistry, MemoryManager memoryManager) {
        this.llmClient = llmClient;
        this.toolRegistry = toolRegistry;
        this.planner = new Planner(llmClient, memoryManager);
    ...
    public String runWithAutoExecute(String userInput) {
        try {
            ...
            if (memoryManager != null) {
                ...
                // 检索相关记忆
                String memoryContext = memoryManager.buildContextForQuery(userInput, 1000);
    ...
    private String executePlan(ExecutionPlan plan) {
        ...
        while (true) {
            ...
            if (batch.size() == 1) {
                ...
                try {
                    ...
                    // 记录任务结果到记忆系统
                    if (memoryManager != null) {
                        memoryManager.addToolResult("task_" + task.getId(), result.content());
                    }
                } catch (Exception e) {
        ...
        // 记录计划完成到记忆系统
        String result = buildFinalResult(plan);
        if (memoryManager != null) {
